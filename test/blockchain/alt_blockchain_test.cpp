...
std::shared_ptr<AltChainParams> altconfig =
    std::make_shared<AltChainParamsTest>();

struct AltTreeTest : public AltTree, public testing::Test {
  ValidationState state;

  AltTreeTest() : AltTree(*altconfig) { bootstrapWithGenesis(state); }
...
TEST_F(AltTreeTest, acceptBlock_test) {
  ...
  AltBlock block1;
  block1.hash = {1, 2, 5};
  block1.previousBlock = config_.getBootstrapBlock().hash;
  block1.height = config_.getBootstrapBlock().height + 1;
  block1.timestamp = 0;
  ...
  AltBlock block2;
  block2.hash = {2, 2, 2};
  block2.previousBlock = config_.getBootstrapBlock().hash;
  block2.height = config_.getBootstrapBlock().height + 1;
  block2.timestamp = 0;
