...
TEST_F(TestStorage, SimplePut) {
  ...
  blockBtc.header = std::make_shared<BtcBlock>(btcBlock1);
  blockBtc.height = 0;
  bool retBtc = repoBtc->put(blockBtc);
  EXPECT_FALSE(retBtc);
  BlockIndex<VbkBlock> blockVbk;
  blockVbk.header = std::make_shared<VbkBlock>(defaultBlockVbk);
...
TEST_F(TestStorage, GetManyByHash) {
  ...
  storedBtcBlock1.header = std::make_shared<BtcBlock>(btcBlock1);
  storedBtcBlock1.height = 0;

  BlockIndex<BtcBlock> storedBtcBlock2;
  storedBtcBlock2.header = std::make_shared<BtcBlock>(btcBlock2);
