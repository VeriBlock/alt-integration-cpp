...
TEST_F(RewardsTestFixture, basicReward_test) {
  ...
  EXPECT_EQ(altchain.at(altchain.size() - 1).getHeight(), 10);
  ...
  EXPECT_EQ(altchain.at(altchain.size() - 1).getHeight(), 11);
  ...
  auto payoutBlockRound =
      sampleCalculator.getRoundForBlockNumber(endorsedBlock.getHeight());
