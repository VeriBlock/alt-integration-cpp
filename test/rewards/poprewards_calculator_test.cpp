...
/*struct PopRewardsParamsNoFlatRound : public PopRewardsParams {
  bool flatScoreRoundUse() const noexcept { return false; }
};

struct AltChainParamsNoFlatRound : public AltChainParamsTest {
 ...
 private:
  std::shared_ptr<PopRewardsParams> popRewardsParams =
      std::make_shared<PopRewardsParamsNoFlatRound>();
};

TEST_F(RewardsCalculatorTestFixture, setReward_test) {
  std::vector<std::pair<PopRewardsBigDecimal, PopRewardsBigDecimal>> values = {
      {1.0, 1.0},
      {2.0, 1.0},
      {5.0, 1.0},
      {10.0, 1.0},
  ...
  AltChainParamsNoFlatRound chainParams2{};
  rewardsCalculator = PopRewardsCalculator(chainParams2);
  for (int height = 5; height < 9; height++) {
    for (const auto& value : values) {
      auto reward = rewardsCalculator.calculateMinerReward(
          height, 0, value.second, value.first);
      fprintf(stderr,
              "Round %lu. Diff %llu, Score %llu. Reward: %llu.%llu\n",
              rewardsCalculator.getRoundForBlockNumber(height),
              value.first.getIntegerFraction(),
              value.second.getIntegerFraction(),
              reward.getIntegerFraction(),
              reward.getDecimalFraction() / reward.decimals);
    }
