...
void sha256_update(sha2_context *ctx, const uint8_t *input, uint32_t ilen) {
    sha2_update(ctx, input, ilen);
}

std::vector<uint8_t> sha256_finish(sha2_context *ctx)
{
    const uint32_t kHashSize = 32;
    unsigned char output[kHashSize] = {};
    sha2_finish(ctx, output);

    //  validate for empty hash 
    bool isEmptyHash = true;
    for (uint32_t i = 0; i < kHashSize-1; i++) {
    ...
    return std::vector<uint8_t>(output, output + kHashSize);
}
...
std::vector<uint8_t> sha256(const uint8_t* buf, size_t nsize)
{
    sha2_context ctx = {};
    sha256_init(&ctx);
    sha256_update(&ctx, buf, nsize & 0xFFFFFFFF);
