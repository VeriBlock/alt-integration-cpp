...
static int getBestPublicationHeight(const BlockIndex<AltBlock>& block,
  ...
  for (const auto& e : block.containingContext) {
    auto* b = vbk_tree.getBlockIndex(e.second.getEndorsement()->blockOfProof);
...
PopRewardsBigDecimal PopRewards::scoreFromEndorsements(
  ...
  for (const auto& e : block.containingContext) {
    auto* b = vbk_tree_.getBlockIndex(e.second.getEndorsement()->blockOfProof);
...
std::map<std::vector<uint8_t>, int64_t> PopRewards::calculatePayouts(
    const BlockIndex<AltBlock>& block, PopRewardsBigDecimal popDifficulty) {
  ...
  for (const auto& e : block.containingContext) {
    ...
    rewards[e.second.getEndorsement()->payoutInfo] +=
        minerReward.value.getLow64();
  }
  return rewards;
}
