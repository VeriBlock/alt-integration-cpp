...
bool altintegration::DeserializeRaw(ReadStream& stream,
  ...
  int32_t numLayers = 0;
  if (!readSingleBEValueNoExcept<int32_t>(stream, numLayers, state)) {
    return state.Invalid("invalid-num-layers");
  }
  if (!checkRangeNoExcept(numLayers, 0, MAX_LAYER_COUNT_MERKLE, state)) {
    return state.Invalid("invalid-num-layers");
  }

  int32_t sizeOfSizeBottomData = 0;
  if (!readSingleBEValueNoExcept<int32_t>(
          stream, sizeOfSizeBottomData, state)) {
  ...
  if (!stream.readBENoExcept<int32_t>(sizeOfBottomData, state)) {
...
bool altintegration::Deserialize(ReadStream& stream,
  ...
  if (!readVarLenValueNoExcept(
          stream, merkleBytes, state, 0, MAX_MERKLE_BYTES)) {
