...
template <typename T>
using Span = altintegration::Slice<T>;
...
/**
 * Implement the Ser and Unser methods needed for implementing a formatter (see
 * Using below).
 *
...
#define FORMATTER_METHODS(cls, obj)                             \
  ...
  static inline void SerializationOps(                          \
      Type& obj, Stream& s, Operation ser_action)

/**
 * Implement the Serialize and Unserialize methods by delegating to a single
 * templated static method that takes the to-be-(de)serialized object as a
 * parameter. This approach has the advantage that the constness of the object
 * becomes a template parameter, and thus allows a single implementation that
 * sees the object as const for serializing and non-const for deserializing,
 * without casts.
 */
...
struct CustomUintFormatter {
  ...
  void Ser(Stream& s, I v) {
    if (v < 0 || v > MAX)
      throw std::ios_base::failure("CustomUintFormatter value out of range");
    if (BigEndian) {
...
struct VectorFormatter {
  ...
  void Unser(Stream& s, V& v) {
    ...
    while (allocated < size) {
      ...
      // attacker actually needs to provide X MiB of data to make us allocate
      // X+5 Mib.
...
// void Serialize_impl(Stream& os, const prevector<N, T>& v, const unsigned
// char&); template <typename Stream, unsigned int N, typename T, typename V>
...
void Serialize(Stream& os, const altintegration::Blob<N>& b) {
  os.write(b);
}

template <typename Stream, size_t N>
void Unserialize(Stream& is, altintegration::Blob<N>& b) {
  unsigned int nSize = ReadCompactSize(is);
  str.resize(nSize);
  if (nSize != 0) is.read((char*)str.data(), nSize * sizeof(C));
}
